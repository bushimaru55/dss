...
- **011 は** **[SPEC-TI-005](../03_analysis_human/SPEC-TI-005-human-review-flow.md)** の **review 状態**・**解決度**・**未解消**／**エスカレーション**・**残課題**を**参照しうる**。**confidence**／**suppression**／**readiness** に反映するが、**005 の結果を無条件の高信頼確定**とはみなさない。  
- **review 済み**を**無条件加点**とはせず、**解決度**・**結果内容**・**残課題**に応じて調整する。**005** は **state／resolution** の管理、**011** は **score／抑制判断**であり**役割が別**である。  
- **011** は **review の状態管理**を担わず、**参照・反映**に留める。**013** は **005 の宣言を尊重**しつつ **011** を補助材料とする前提を**壊さない**。  
- 005 側の整理は **[SPEC-TI-005](../03_analysis_human/SPEC-TI-005-human-review-flow.md)** の「011 信頼度スコアリングへの受け渡し（MVP 境界）」を参照。

### 013 分析候補生成への受け渡し（MVP 境界）

- **011** の **confidence**／**readiness**／**caution**（および **explanation**）は **013** の**候補提示制御**（**提示可否**・**優先度**・**注意付き提示**・**限定提示**）に**参照されうる**。  
- ただし **005** の **`suggestion_suppression_level`** および **review** 由来の制約を **011 が上書きするものではない**。**013** は **005 を尊重**し、**011** を**補助入力**とする。  
- **011** は**候補採択の正本**ではなく、**候補提示の補助**である。**数値や状態だけ**で機械的に採択を**確定**しないのは **013** 側の整理（**自動確定しない**）。  
- 013 側の整理は **[SPEC-TI-013](../03_analysis_human/SPEC-TI-013-suggestion-generation.md)** の「011 信頼度スコアリングとの接続境界（MVP）」を参照。
...
- **0.3（2026-04-19）**: **`suggestion_suppression_level` は 005 正本**、011 は確定せず信号／推奨、013 は 005 尊重＋011 補助で最終制御、境界の再掲更新。  
- **0.3.1（2026-04-07）**: §関連仕様書に 005 との MVP 接続境界を追記。  
- **0.3.2（2026-04-07）**: §関連仕様書に 013 との MVP 接続境界を追記。**現行 Draft**（詳細は下表）。

| 版 | 日付 | 概要 |
|----|------|------|
| 0.3.2 | 2026-04-07 | §関連仕様書に「013 分析候補生成への受け渡し（MVP 境界）」を追記（confidence／readiness／caution の補助性・005 上書き禁止・013 節への参照）。 |
| 0.3.1 | 2026-04-07 | §関連仕様書に「005 人確認結果との接続境界（MVP）」を追記（解決度・無条件加点禁止・005／011 役割分離・013 整合・005 節への参照）。 |
