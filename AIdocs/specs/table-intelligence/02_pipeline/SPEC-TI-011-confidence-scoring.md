...
- **011 はスコア値・閾値・説明構造の正本**である。**005 の解消状態**および **013 の `suggestion_suppression_level` 前提**と**整合**する（013 は 011 を**上書きしない**。**判断材料**として用いる）。  
...
4. **013 連携**: **低スコア**と **`suggestion_suppression_level`** を**一貫**させる（013 が最終の候補集合を決定）。  

**境界の再掲**: **011＝信頼度**、**013＝候補生成**、**007＝表示**。
...
- 003 **`type_normalization_notes[]`**, `incomplete_bindings[]`, `skipped_regions[]`  
- 004 **`inferred_business_meaning`** の有無・弱さフラグ
...
| **003** | **`normalization_status`**（COMPLETE／PARTIAL／FAILED）、`incomplete_bindings`／`skipped_regions` **件数**、**`type_normalization_notes`**。 |
| **004** | **`review_required`**、`review_points` **件数**・**最大 severity**、`metadata_confidence_hints`、**inferred** の弱さ／空。 |
...
- 003 **`PARTIAL`／`FAILED`**、**incomplete_bindings**、**型パース失敗**の多さ。  
...
- [SPEC-TI-005](../03_analysis_human/SPEC-TI-005-human-review-flow.md) の **`suggestion_suppression_level`** は **005 が宣言する前提**。011 は **スコアから同レベルを**推奨しうるが、**矛盾**した場合は **explanation に両方を記録**し、**013 が最終調整**（013 正本）。  
- **暫定マッピング（例）**  
...
**011 は 013 を上書きしない**。**判断材料**としてスコアと explanation を渡す。
...
- **集約スコア**と **`suggestion_suppression_level`** の**最終合成**（005 宣言との**優先順位**）。  
...
| 版 | 日付 | 概要 |
|----|------|------|
| 0.1 | 2026-04-08 | 初版本文。粒度、特徴量、加減点、review grade、decision／抑制、explanation、013 関係。 |
