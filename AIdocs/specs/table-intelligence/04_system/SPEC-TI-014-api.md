...
| `GET .../candidates` | `candidates` + **`generation_constraints_reference`**（detail と同系 read-only）+ 条件付き `decision_recommendation`（011 JSON）。 |
...
## 19. MVP 読み取り専用参照面（OpenAPI 0.1.9〜0.1.14）

実装済み backend（Django）が返す **観測用 JSON** と OpenAPI スキーマの対応。いずれも **新しい正本を定義しない**。**`semantic_lock_in: false`** を前提とする。

| リソース（GET 200） | 追加フィールド | 正本／補助の境界 |
|---------------------|----------------|-------------------|
| `TiMvpHumanReviewSession` | `mvp_005_canonical_summary` | **005**: `HumanReviewSession` / `SuppressionRecord` / snapshot の観測派生。自動解決しない。 |
| `TiMvpConfidenceEvaluation` | `review_state_reference` | **011** の `confidence_score` 等は不変。内包する 005 サマリ参照は **読み取り専用**（005 が一次）。 |
| `TiMvpSuggestionSet` | `generation_constraints_reference` | **013**: `primary_constraints_from_005`（一次制約として**参照のみ**）、`auxiliary_signals_from_011`（補助）。**候補配列を自動変更しない**。 |

### 19.2 011 補助 signal の参照整合（Pattern A）

- **`GET /evaluations/{evaluation_ref}`** のトップレベル **`confidence_score` / `risk_signals` / `decision_recommendation`** は **011 正本**（`ConfidenceEvaluation` 行）。**`risk_signals`** は JSON **配列**で、**要素は object を含みうる**（**string[] 固定ではない**）。**strict JSON Schema 化は未着手**（OpenAPI は `items: {}` 寄り）。  
- **`GET /suggestion-runs/{ref}`** および **`GET /suggestion-runs/{ref}/candidates`** の **`generation_constraints_reference.auxiliary_signals_from_011`** は、同一 `metadata` の**最新** 011 評価について、上記フィールドの **read-only 写し**（**`risk_signals` 含む**）。013 は **011 にない signal を捏造しない**。  
- **`review_state_reference`** は 005 観測の添付であり、011 本体値の上書きではない（§19 表どおり）。

### 19.3 005 未解決観測の参照（read-only）

- **`mvp_005_canonical_summary.unresolved_work_present`** 等の **未解決観測**は、**evaluation**（`review_state_reference.mvp_005_canonical_summary`）、**suggestion detail / candidates**（`generation_constraints_reference.primary_constraints_from_005`）、**review session detail**（`mvp_005_canonical_summary`）で **005 由来として整合して見える**ことを意図する。  
- **まだルール化はしない**（§20 参照）。**strict schema / gating / readiness / caution の本格化は未着手**。013 は未解決を **新しい候補ルールにしない**。

### 19.4 caution / readiness 的な読み方（非正規化ガイド）

<a id="s194-caution-readiness-reading-guide"></a>

本項は **API 契約の追加定義ではない**。**利用者が既存レスポンスをどう眺めるとよいか**の補助であり、**閾値・列挙・policy は定めない**（§20 参照）。

#### 位置づけ（新フィールドはない）

- **MVP 現在版**では、**`caution` または `readiness` という名前の、正式な単一 API フィールドは導入していない**。
- 以下の **caution / readiness** は、005 / 011 / 013 に既にある signal を **補助的に解釈するための語彙**に留まる。**正式な単一指標ではない**。
- **strict enum・閾値・自動ポリシー**は **未着手**である。

#### caution 的に読める既存材料（いずれも「注意が要りうる」情報の**材料**）

利用者やクライアントは、必要に応じて次を **あわせて**参照するとよい（**どれか 1 つを「caution フラグ」とみなすことはできない**）。

| フィールド（例） | 主な所在 | 読み方の例 |
|------------------|----------|------------|
| **`risk_signals`** | 011（`GET /evaluations/...` および `auxiliary_signals_from_011`） | 011 が保持する**補助的なリスク情報**。**要素の形は MVP で固定しない**。 |
| **`decision_recommendation`** | 011（同上） | JSON。**`level` 等があれば**、それを **推奨の強さ・種類の示唆**として読める場合がある（**スキーマ固定ではない**）。 |
| **`unresolved_work_present`** | 005 観測（`mvp_005_canonical_summary` / `primary_constraints_from_005` 等） | **人確認作業が未完了である可能性**の観測。**ビジネス上の確定や昇格根拠にはならない**（§19.3）。 |
| **`uncertainty_intake_present`** | 005 観測（同上） | 取り込み時点で **004 由来の不確実性フラグ／論点**があったことの観測。 |
| **`suppression_status`**（および **`suppression_record_count`**） | 005 観測（同上） | **抑制が掛かっている可能性**の観測（正本の解釈は 005）。 |
| **`resolution_status`** / **`resolution_grade`** | 005 観測（同上） | **セッションの粗い区分**および任意の grade。**単独で「安全／危険」を決めない**。 |

**これらのどれか 1 つ = caution ではない**。組み合わせと文脈で解釈する。

#### readiness 的に読める既存材料（「準備度」の**示唆**）

**readiness も、API 上の単一の正式指標ではない**（将来の本格化は §20 および各 SPEC の対象外として残す）。

| フィールド・文脈 | 主な所在 | 読み方の例 |
|------------------|----------|------------|
| **`confidence_score`** | 011 | **信頼度のスカラー**。高い／低いは **011 の評価出力**であり、**実行可否の保証ではない**。 |
| **`decision_recommendation`** | 011 | 推奨ブロック。**「どの程度進めてよいか」の示唆の材料**になりうるが、**固定 enum ではない**。 |
| **005 由来の `unresolved_work_present` / `uncertainty_intake_present`** | 005 観測 | **下流に進む前に人確認・整理が残りうる**ことの材料。 |
| **`analysis_candidates[].readiness`**（該当する場合） | 013 の候補オブジェクト | **MVP では stub 等の例がありうる**。**パイプライン全体の readiness や 011 の評価 readiness と同一視しない**（候補ごとのローカルな表現に過ぎない場合がある）。 |

#### 層ごとの責務（読み方の整理）

- **005**: **未解決・不確実性取り込み・抑制の有無**など、**正本由来の観測**（canonical summary は派生）。  
- **011**: **`confidence_score` / `risk_signals` / `decision_recommendation`** による**補助的な信頼度・リスク・推奨**（002 の `decision` とは別）。  
- **013**: **`generation_constraints_reference`** で 005 / 011 を **read-only に並べた候補生成文脈**。**新たな判定を API に足さない**。

#### 本節でまだやらないこと（次の gating 整理との切り分け）

次は **別途**（§20）論じる前提とし、**本節では定義も実装も行わない**。

- **`caution` / `readiness` の API フィールド化**や **新しい schema 行の追加**
- **gating** のルール化・本格化
- **suppression / 候補昇格**ロジックへの接続
- **`semantic_lock_in` の導入**
- **strict schema / strict enum** 化
- **013 の stub `readiness` の語彙・判定の正式化**（全体指標への昇格を含む）

### 19.5 stub `analysis_candidates` の根拠トレース（004 観測・Pattern B）

<a id="s195-stub-analysis-candidates-004-trace"></a>

- **`POST /suggestion-runs`** で生成される MVP **stub 候補**（`category: summary_stub` 等）は、**件数・カテゴリ・priority・readiness の決め方は従来どおり**（`measures` 空/non-empty のみ）。**gating・候補の意味確定ではない**。
- 各候補の **`evidence` / `risk_notes`** には、**004 `AnalysisMetadata` 由来の観測トレース**（例: `metadata_id`、`dataset_id`、`dimensions[]` / `measures[]` の id・name、`time_axis` の有無）が載りうる。**「なぜこの stub が出たか」を説明するためのもの**であり、**正式な単一指標や確定ルールではない**。
- **005 / 011 は候補の選定・除外に使わない**。005/011 の read-only 情報は **`generation_constraints_reference`**（GET）で**別途**参照する。**候補配列をそのブロックで書き換えない**。
- **`GET /suggestion-runs/...` / `GET .../candidates`** の **`candidates[]` / `analysis_candidates[]`** は同一ストアの投影である。**OpenAPI 0.1.14-draft** の `TiMvpSuggestionSet` / `SuggestionCandidate` description を参照。

### 19.6 observation trace と logical path の区別

<a id="s196-observation-trace-vs-logical-path"></a>

MVP 現在版において、stub 候補の **`evidence[]`** に載る参照は **observation trace**（観測トレース）として扱う。これは **「stub 候補を出すにあたり、004 メタデータ上で何を見たか」**の説明用であり、**006 で想定される論理パス（logical path）や入力要件の正本ではない**。両者を**同一視しない**。

#### observation trace として 013 evidence に安全に含まれうるもの

次は **004 `AnalysisMetadata` の観測の写し／参照**であり、**意味確定・パス確定・semantic lock-in ではない**。

- **`metadata_id`** / **`dataset_id`**（システム ID）
- **`dimensions[]` / `measures[]`** 要素の **id / key / name 相当**（列挙・ラベル写し）
- **`time_axis`** の **有無**（オブジェクトが載っている事実。中身の解釈はここでは確定しない）

#### まだ 006 logical path として固定していないもの（未固定領域）

次は **[SPEC-TI-006](../01_foundation/SPEC-TI-006-io-data.md)** で **論理参照**として扱いうるが、**MVP では path 正本として API に固定していない**。

- **`measures.*` / `dimensions.*` 形式の論理パス記法**（例: `measures.amount`）
- **`required_fields` / `affected_fields`** に載せる **path の正本**（**未実装**／将来の整理対象）
- **`grain`** — SPEC 上は重要概念だが、**current DB / API / metadata 実装では単一の安定した露出箇所に揃っていない**場合がある。**将来の path / requirement 設計で揃える対象**として扱い、本節では**欠陥の断定をしない**。
- **003 / 004 / 006 をまたぐ単一の参照命名規約**（未固定）

#### 対照（読み取りの目安）

| 項目 | 現在の位置づけ |
|------|----------------|
| `metadata_id` / `dataset_id` | **observation trace**（システム ID） |
| `dimensions[]` / `measures[]` の id / key / name | **metadata 観測参照**（trace）。論理パス確定ではない |
| `time_axis` の有無 | **観測事実** |
| `required_fields` | **未実装 / 未固定**（将来は 006 論理参照候補） |
| **006 logical path** | **将来の論理参照単位**（MVP では path 正本にしない） |
| **grain** | **重要概念**だが、**current 実装の正本位置は未固定**になりうる（整理は後続） |

#### 読み手向けの注意

- **`evidence` は候補の trace** であり、**入力必須条件・未入力エラー条件・論理依存を断定するものではない**。
- 将来 **`required_fields`** を実装する場合でも、**今の `evidence` をそのまま path 正本や 006 の確定スキーマとして扱うとは限らない**（**別イテレーションで対応表を作る**前提）。
- **013 は観測の説明に留まり**、**意味確定を行わない**。**005 正本 / 011 補助 / 013 参照**の境界は [§20](#20-未着手境界と次実装前提) および §19.5 と整合する。

### 19.1 最小レスポンス例（正常系）

以下は **理解補助用のダミー値**であり、仕様の拡張や列挙の固定を意味しない。`risk_signals` は **JSON 配列**（要素がオブジェクトになり得る）。**`semantic_lock_in` は MVP で `false`**。

#### `GET /review-sessions/{session_id}`（`TiMvpHumanReviewSession`）

`mvp_005_canonical_summary` は **常に object**（005 正本の観測用派生。自動解決しない）。
...
```json
{
  ...
  "resolution_grade": null,
  "mvp_005_canonical_summary": {
    "schema_ref": "ti.mvp_005_canonical_review_summary.v1",
    ...
    "unresolved_work_present": true,
    "uncertainty_intake_present": true,
    "from_004_review_required_snapshot": true,
    "from_004_review_point_count_at_intake": 0
  },
  "created_by_id": 42,
...
#### `GET /evaluations/{evaluation_ref}`（`TiMvpConfidenceEvaluation`）

011 本体（`confidence_score` / `risk_signals` / `decision_recommendation`）は **正本のまま**。`review_state_reference` は **常に object**；内包する 005 サマリは **参照面**（011 が 005 を上書きしない）。同一 `metadata` にセッションがある場合の **正常系**例。

```json
{
  "evaluation_ref": "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa",
  "metadata_id": "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb",
  "workspace_id": "ws-demo",
  "confidence_score": 0.72,
  "risk_signals": [{ "code": "sample_noise" }],
  "decision_recommendation": { "level": "review", "source": "011" },
  "review_state_reference": {
    "schema_ref": "ti.mvp_011_review_state_reference.v1",
    "semantic_lock_in": false,
...
参照先セッションが無い場合は、**`review_state_reference.mvp_005_canonical_summary` が `null` になり得る**（トップの `review_state_reference` キー自体は存在）。

#### `GET /suggestion-runs/{suggestion_run_ref}`（`TiMvpSuggestionSet`）

`generation_constraints_reference` は **常に object**。`primary_constraints_from_005` は一次制約としての **005 観測**、`auxiliary_signals_from_011` は **011 の補助**（再計算しない）。**013 は参照のみ**（候補配列や suppression 正本をここで再定義しない）。

```json
{
  "suggestion_run_id": "dddddddd-dddd-4ddd-8ddd-dddddddddddd",
  "table_id": "eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee",
  "analysis_candidates": [{ "id": "c1", "category": "summary_stub" }],
  "suppression_applied": [],
  "metadata_id": "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb",
  "generation_constraints_reference": {
    "schema_ref": "ti.mvp_013_generation_constraints_reference.v1",
    "semantic_lock_in": false,
    "primary_constraints_from_005": {
      "schema_ref": "ti.mvp_005_canonical_review_summary.v1",
      "semantic_lock_in": false,
      "review_state": "OPEN",
      "resolution_status": "PENDING",
...
該当する 005 セッション／011 評価が無い場合は、**`primary_constraints_from_005` / `auxiliary_signals_from_011` が `null` になり得る**（トップの `generation_constraints_reference` キー自体は存在）。

機械可読定義および **schema 上の `example`** はリポジトリ内 **[OpenAPI 叩き台](./openapi/table-intelligence-openapi-draft.yaml)**（**0.1.14-draft**；§19.2・**`risk_signals` / `auxiliary_signals_from_011` の契約補足**、§19.3・**005 未解決観測**、§19.5・**stub `analysis_candidates` の 004 根拠トレース**）の `Mvp005CanonicalSummary` / `Mvp011ReviewStateReference` / `Mvp013GenerationConstraintsReference` および `TiMvp*` / `SuggestionCandidate` を正とする。**§19.6**（**observation trace と 006 logical path の区別**）は **AIdocs のみ**の整理であり、OpenAPI 本文の追加変更を伴わない。詳細な語彙は [SPEC-TI-005](../03_analysis_human/SPEC-TI-005-human-review-flow.md)、[SPEC-TI-011](../02_pipeline/SPEC-TI-011-confidence-scoring.md)、[SPEC-TI-013](../03_analysis_human/SPEC-TI-013-suggestion-generation.md) の **Backend MVP API** 節を参照。

---

## 20. 未着手境界と次実装前提

MVP 収束後の **read-only 参照面**（§19）と OpenAPI まで整合した時点で、**次の backend 実装に入る前**に、触ってよい範囲と **まだ手を付けない論点**をここで区別する。本節は **仕様の大改稿ではなく**、実装前の**境界の明文化**である。以下の「次実装候補」は **列挙にすぎず**、どれをいつ採用するかは **別途決定**する（本節で確定仕様とはしない）。

### 20.1 この時点で固定している責務境界（維持対象）

- **005** が人確認・**suppression 等の正本**である。
- **011** は信頼度・推奨の**補助**であり、**005 正本を上書きしない**。
- **013** は候補生成において 005／011 を **read-only で参照**するにとどまり、**正本状態をここから書き換えない**。
- **`mvp_005_canonical_summary` 等の canonical 系 JSON** は **正本ではなく観測用派生**である。
- **`semantic_lock_in`** は MVP では **常に `false`**（導入・意味付けは **未着手**）。

### 20.2 まだ本格化していない論点（未着手）

次の論点は、**意図的に未着手**である。ルール本文・API 契約の確定は **本節では行わない**。

- **gating**（提示・実行のゲート）ルールの本格化。
- **caution / readiness** の詳細ルール化。
- **未解決（unresolved）** を候補昇格・確定にどう作用させるかの**詳細化**（現状、read-only 参照面は **根拠として昇格を確定しない**）。
- **候補 API の追加 query・候補 item への参照面の重複載せ**等、§19 参照面の**さらなる展開**（**`GET .../candidates` トップへの `generation_constraints_reference` 同梱**は read-only で対応済み—OpenAPI **0.1.11-draft**）。
- **011 → 013** の補助信号の**拡張**（フィールド・計算・契約）。
- **`Mvp005` / `Mvp011` / `Mvp013` 各ブロックの厳密 JSON Schema 化**（現状は `additionalProperties` 中心のドラフト）。
- **`semantic_lock_in` の導入**およびそれに紐づく意味。

### 20.3 次の実装で検討しうる候補（未確定・優先は未割当）

以下は **後続で実装候補になり得る**テーマの例である。**いずれも本書のこの版では仕様確定しない**。

- 013 側の **最小 gating**（導入の有無・閾値・表現は未確定）。
- **caution / readiness** の **最小**な表出（詳細定義は未着手のまま）。
- suggestion **候補系 API** への参照面の**さらなる展開**（**candidates GET への同型ブロック同梱**は済。item 単位・追加 query は未着手）。
- 011 由来の **補助信号の追加**（どのフィールドかは未確定）。
- **005 の未解決状態**と下流（013／UI）の**接続の整理**。

### 20.4 本節の位置づけ

- **実装開始直前の地ならし**として、**「今はここまで固定・ここから先は未着手」**を API 文書上の**主置き場**（本節）に集約した。
- 次のイテレーションで **どの候補から着手するか**は、プロダクト／実装側で選び、**必要なら** 005／011／013 各 SPEC または OpenAPI を **個別に**追補する（本節はその前提台帳である）。
...
| 0.1 追補 15 | 2026-04-08 | **§19**: MVP 読み取り専用参照面（`mvp_005_canonical_summary` / `review_state_reference` / `generation_constraints_reference`）。OpenAPI **0.1.9-draft**。005／011／013 の SPEC に Backend MVP API 節を追補。 |
| 0.1 追補 16 | 2026-04-08 | **§19.1**: read-only 参照面の **最小レスポンス例**（正常系のみ）。OpenAPI **0.1.10-draft**: `TiMvpHumanReviewSession` / `TiMvpConfidenceEvaluation` / `TiMvpSuggestionSet` に **schema `example`**、`Mvp011` / `Mvp013` の **nullability 補足**（description のみ）。 |
| 0.1 追補 17 | 2026-04-08 | **§20**: MVP 後続実装に向けた **未着手境界**（固定済み責務／未着手論点／次候補の非確定列挙／本節の位置づけ）。実装前の整理であり仕様大改稿ではない。 |
| 0.1 追補 18 | 2026-04-08 | **`GET .../candidates`**: 応答に **`generation_constraints_reference`**（detail と同型・read-only）を同梱。OpenAPI **0.1.11-draft**。§18.3 表・§20.2/20.3 を最小更新。 |
| 0.1 追補 19 | 2026-04-08 | **§19.2**: 011 補助 signal（**`risk_signals` 等**）と 013 **`auxiliary_signals_from_011`** の **read-only 写し**を明文化（Pattern A）。OpenAPI **0.1.12-draft**。SPEC-TI-011 / 013 追記。 |
| 0.1 追補 20 | 2026-04-08 | **§19.3**: 005 **`unresolved_work_present`** 等の **read-only 参照整合**（evaluation / suggestion detail / candidates / session）。OpenAPI **0.1.13-draft**（`Mvp005` / `Mvp011` / `Mvp013` description 補足）。SPEC-TI-005 / 011 / 013 追記。 |
| 0.1 追補 21 | 2026-04-08 | **§19.4**: caution / readiness を**新フィールド化せず**、既存 signal の**利用者向け読み方ガイド**（非正規化）。gating 前の下地。 |
| 0.1 追補 22 | 2026-04-08 | **§19.5**: stub **`analysis_candidates`** の **004 観測トレース**（`evidence` / `risk_notes`）。OpenAPI **0.1.14-draft**。SPEC-TI-013 Pattern B 追記。 |
| 0.1 追補 23 | 2026-04-08 | **§19.6**: **observation trace** と **006 logical path** の区別（013 `evidence` は path 正本ではない）。AIdocs のみ。OpenAPI 変更なし。 |
